...
class ProductController @Inject()(cc: ControllerComponents, actorSystem: ActorSystem)(implicit ec: ExecutionContext) extends AbstractController(cc) {
  ...
  def getProduct(id: String): Action[AnyContent] = Action.async {
    println("Get Product with id: ", id)
